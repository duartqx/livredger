...
func (u *UnidadeDeTrabalhoSqlite) GetTransaction() *sql.Tx {
	if u.Tx == nil {
		panic(fmt.Errorf("%w UnidadeDeTrabalho: Já existe uma transação aberta", types.InternalError))
	}
	return u.Tx
}
...
func (u *UnidadeDeTrabalhoSqlite) BeginTransaction() (tx *sql.Tx, err error) {
	if u.Tx != nil {
		return nil, fmt.Errorf("%w UnidadeDeTrabalho: Já existe uma transação aberta", types.InternalError)
	}

	tx, err = u.DB.Begin()

	if err != nil {
		return nil, fmt.Errorf("%w UnidadeDeTrabalho: Não foi possível iniciar uma transação (%w)", types.InternalError, err)
	}
...
func (u *UnidadeDeTrabalhoSqlite) Commit() error {
	if u.Tx == nil {
		return fmt.Errorf("%w UnidadeDeTrabalho: Nenhuma transação aberta", types.InternalError)
	}

	if err := u.Tx.Commit(); err != nil {

		if match := regex.SqliteFalhouCommitar.FindStringSubmatch(err.Error()); len(match) > 1 {
			return fmt.Errorf("%w: %s", types.BusinessLogicError, match[1])
		}

		return fmt.Errorf("%w UnidadeDeTrabalho: Não foi possível commitar a transação (%w)", types.InternalError, err)
	}
...
func (u *UnidadeDeTrabalhoSqlite) Rollback() error {
	...
	if err := u.Tx.Rollback(); err != nil {
		return fmt.Errorf("%w UnidadeDeTrabalho: Não foi possível fazer rollback (%w)", types.InternalError, err)
	}
