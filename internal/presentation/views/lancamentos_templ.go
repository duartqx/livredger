...
func ConsultaLancamentos() templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		ctx = templ.ClearChildren(ctx)
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("<lancamentos x-data=\"ConsultaLancamentos\"><template x-for=\"(lancamento, index) in lancamentos\" :key=\"`${lancamento.id}-${lancamento.chave}`\"><lancamento class=\"d-flex justify-content-around bg-body-tertiary p-3 my-4\"><span x-text=\"dayjs.utc(lancamento.timestamp).tz(&#39;America/Sao_Paulo&#39;).format(&#39;LLLL&#39;)\"></span> <span x-text=\"lancamento.evento\"></span> <span x-text=\"lancamento.valores\"></span> <span x-text=\"lancamento.natureza\"></span> <span x-text=\"lancamento.descricao\"></span> <span x-text=\"dayjs.utc(lancamento.vencimento).tz(&#39;America/Sao_Paulo&#39;).format(&#39;LLLL&#39;)\"></span></lancamento></template></lancamentos>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return templ_7745c5c3_Err
